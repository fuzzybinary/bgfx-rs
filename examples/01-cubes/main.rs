...
use bgfx::*;
use cgmath::{
  Angle, Decomposed, Deg, Matrix4, Point3, Quaternion, Rad, Rotation3, Transform, Vector3,
};
...
impl<'a> Cubes<'a> {
  ...
  fn init(&mut self) {
    ...
    self.vbh = Some(VertexBuffer::new(
      Memory::reference(self.bgfx, &CUBE_VERTICES),
      &decl,
      BUFFER_NONE,
    ));

    // Create static index buffer.
    self.ibh = Some(IndexBuffer::new(
      Memory::reference(self.bgfx, &CUBE_INDICES),
      BUFFER_NONE,
    ));

    // Create program from shaders.
    self.program = Some(examples_lib::load_program(
      &self.bgfx, "vs_cubes", "fs_cubes",
    ));
  ...
  fn update(&mut self) -> bool {
    if !self
      .events
      .handle_events(&self.bgfx, &mut self.width, &mut self.height, self.reset)
    {
      let now = PreciseTime::now();
      let frame_time = self.last.unwrap_or(now).to(now);
      self.last = Some(now);

      let time = (self.time.unwrap().to(now).num_microseconds().unwrap() as f64) / 1_000_000.0_f64;
      ...
      self
        .bgfx
        .dbg_text_print(0, 1, 0x4f, "examples/01-cubes/main.rs");
      self
        .bgfx
        .dbg_text_print(0, 2, 0x6f, "Description: Rendering simple static mesh.");
      ...
      self
        .bgfx
        .set_view_transform(0, view.as_ref(), proj.as_ref());
      ...
      for yy in 0..11 {
        for xx in 0..11 {
          ...
          modifier.rot = Quaternion::from_euler(
            Rad::new((time / 0.21) as f32),
            Rad::new((time / 0.37) as f32),
            Rad::new(0.0),
          );
          modifier.disp = Vector3::new(-15.0 + (xx as f32) * 3.0, -15.0 + (yy as f32) * 3.0, 0.0);
